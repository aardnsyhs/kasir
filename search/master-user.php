...
$sql = "SELECT * FROM `sulistyo_user` 
        JOIN role 
        ON sulistyo_user.id_role = role.id_role
        WHERE LOWER(username) LIKE '%$keyword%'
        OR LOWER(nama_user) LIKE '%$keyword%'
        OR LOWER(id_user) LIKE '%$keyword%'
        OR LOWER(nama_role) LIKE '%$keyword%'
        ";
$user = mysqli_query($conn, $sql);
?>

<table class="table table-fluid table-bordered table-striped w-50">
    <thead>
    ...
    <tbody>
        <?php foreach ($user as $sulistyo_row): ?>
            <tr class="text-center align-middle">
